...
def demo(sess, net, image_name, inDir, featDir, oFeatDir, outDir):
    ...
    feat_keys = ['gt_box', 'ss_box', 'pos_id', 'gt_pos_all', 'bbx_reg_all', 'bbx_reg', 'sen_lang', 'context_triplet', 'sen_ids', 'height', 'width', 'sen_global_token', 'sens', 'sen_lang_token']
        ...
        if key == 'ss_box':
            ...
            for pos_ind, pos_ids in enumerate(pos_ids_all):
                if pos_ids == []:
                    ...
                    gt_box = cur_sen_feat['gt_box'][pos_ind].astype('float')
            ...
            for pos_ind, pos_id in enumerate(pos_ids):
                if pos_id == -1:
                    ...
                    gt_box = cur_sen_feat['gt_box'][pos_ind]
            ...
            cur_sen_feat = {}
            oFeat = feat_file['gt_box']
            cur_sen_feat['gt_box'] = oFeat
            pickle.dump(cur_sen_feat, open(out_file, 'wb'), protocol=pickle.HIGHEST_PROTOCOL)    
        else:
            cur_sen_feat = np.load(out_file)
...
def parse_args():
    ...
    parser.add_argument('--threadID', dest='threadId', help='Thread ID for extract',
                        default=0)
    parser.add_argument('--testFile', dest='flickrtest', help='Flickr30k test IDs',
                        default=None)
...
if __name__ == '__main__':
    ...
    flickrtestIDs = np.loadtxt(args.flickrtest, dtype=np.object)
    #flickrtestIDs = np.array(os.listdir(args.inputDir))
