...
def tokenize_img_cap(sens, word_dict):
    ...
    for sen in sens:
        ...
        for word in words:
            print word
            token.append(np.where(word_dict==word)[0][0])
        tokens.append(token)
    return tokens
...
def calc_spat_feat(img_list, res_dir):
    for img_ind, img_id in enumerate(img_list):
        ...
        spat_feat = np.zeros((ss.shape[0], 8))
        spat_feat[:, :4] = ss.astype('float')        # unnormalized [xmin, ymin, xmax, ymax]
...
def add_img_ss_label(img_list, gt_dir, proposal_dir, res_dir, temp_feat=None):
    for img_ind, img_id in enumerate(img_list):
        print '%d/%d: %s'%(img_ind+1, len(img_list), img_id)        
...
def gen_img_cap_feat2(img_list, gt_file, proposal_dir, sen_dir, res_dir):
    ...
    for img_ind, img_id in enumerate(img_list):
        ...
        gt_boxes = cur_sen_feat['gt_loc_all']
        word_dict = np.load('word_dict_referit.npy')
...
def add_img_gt_label(img_list, gt_file, proposal_dir, res_dir, queries=None):
    for img_ind, img_id in enumerate(img_list):
        ...
        cur_sen_feat['gt_loc_all'] = gt_box
...
def add_img_sen_lang_token(img_list, gt_dir, proposal_dir, res_dir):
    ...
    for img_ind, img_id in enumerate(img_list):
        ...
        for pos_ind, pos_id in enumerate(pos_ids):
            if pos_id == -1:
                ...
                gt_box = cur_sen_feat['gt_loc_all'][pos_ind]    # [xmin, ymin, xmax, ymax]
    ...
    for img_ind, img_id in enumerate(img_list):
        ...
        for pos_ind, pos_ids in enumerate(pos_ids_all):
            if pos_ids == []:
                ...
                gt_box = cur_sen_feat['gt_loc_all'][pos_ind].astype('float')    # [xmin, ymin, xmax, ymax]
